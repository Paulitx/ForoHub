...
public class UsuarioController {
    ...
    public ResponseEntity actualizarUsuario(@RequestBody @Valid DatosActualizarUsuario datosActualizarUsuario){
        ...
        usuario1 = usuarioRepository.getReferenceById(datosActualizarUsuario.id());
        if(datosActualizarUsuario.clave() != null){
            String claveEncriptada = encoder.encode(datosActualizarUsuario.clave());
            usuario1.actualizarDatosClave(datosActualizarUsuario, claveEncriptada);
        } else {
             usuario1.actualizarDatos(datosActualizarUsuario);
        }
        return ResponseEntity.ok(new DatosRespuestaUsuario(usuario1.getId(), usuario1.getNombre(), usuario1.getEmail(),
                usuario1.getStatus()));
    }
